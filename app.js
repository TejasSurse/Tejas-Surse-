...
app.get("/bbb", async (req, res) => {
    try {
        // Fetch all messages from the database using the Contact model
        const messages = await Contact.find().sort({ receivedAt: -1 });
        // Pass the retrieved messages to the EJS template
...
app.post('/projectUpload', upload.single('banner'), async (req, res) => {
    try {
        ...
        const bannerUpload = await uploadOnCloudinary(fileBuffer, { folder: "portfolio_projects" });
